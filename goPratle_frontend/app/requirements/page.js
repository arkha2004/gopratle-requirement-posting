	...
	function validateCurrentStep() {
		...
		if (currentStep === 2) {
			...
			if (formData.category === "Event Planner") {
				...
				} else if (Number(formData.guestCount) <= 0) {
					newError.guestCount = "Guest Count cannot be zero or negative";
				}

				if (!formData.budget) {
					newError.budget = "Budget is required";
				} else if (Number(formData.budget) <= 0) {
					newError.budget = "Budget cannot be zero or negative";
				}
			}
			...
			if (formData.category === "Performer") {
				if (!formData.performanceType.trim()) {
					newError.performanceType = "Performance Type is required";
				}

				if (!formData.performerCount) {
					newError.performerCount = "Number of Performers is required";
				} else if (Number(formData.performerCount) <= 0) {
					newError.performerCount =
						"Performer Count cannot be zero or negative";
				}

				if (!formData.performanceDuration.trim()) {
					newError.performanceDuration = "Performance Duration is required";
				} else if (Number(formData.performanceDuration) < 0) {
					newError.performanceDuration =
						"Performance Duration cannot be zero or negative";
				}
			}
			...
			if (formData.category === "Crew") {
				...
				if (!formData.crewCount) {
					newError.crewCount = "Number of Crew Members is required";
				} else if (Number(formData.crewCount) <= 0) {
					newError.crewCount = "Crew Count cannot be zero or negative";
				}

				if (!formData.experienceRequired.trim()) {
					newError.experienceRequired = "Experience Required is required";
				}
			}
		}
		setErrors(newError);
